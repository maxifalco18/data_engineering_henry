...
proyecto_integrador/
├── notebooks/
│   └── 2_analisis_de_ventas.ipynb   # <-- Notebook de integración y visualización.
├── src/
│   ├── database.py                # <-- Gestor de conexión (Implementa Singleton).
│   ├── models.py                  # (Sin cambios estructurales desde la Fase 1).
│   └── repository.py              # <-- Capa de acceso a datos (Implementa Repository).
├── tests/
│   ├── test_database.py           # <-- Prueba unitaria para el patrón Singleton.
│   └── test_repository.py         # <-- Prueba unitaria para el Repository (con Mocks).
├── .env                           # <-- Archivo para credenciales (local, no en Git).
...
├── src/
│   └── repository.py              # <-- Actualizado con nuevos métodos para llamar a las queries avanzadas.
├── sql/
│   └── 3_advanced_objects.sql     # <-- NUEVO: Script para crear la Vista y el Procedimiento Almacenado.
